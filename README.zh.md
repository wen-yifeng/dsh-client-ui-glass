...
DeepSeek Harness（DSH）Web 客户端的自包含可开关玻璃拟态（glassmorphism）皮肤，移植自无限智能画布「单一母材质」体系（光晕舞台、四层材质、Tier A 顶级交互）。插件是唯一决策者：在原版 DSH 与已消费其 token 契约的构建上表现一致。

## 截图

![会话视图（背板光晕开启）](preview_1.png)
![设置（背板光晕开启）](preview_2.png)
![会话视图（纯黑背板）](preview_3.png)
![设置（背板光晕关闭）](preview_4.png)
