...
A self-contained, toggleable glassmorphism skin for the DeepSeek Harness (DSH) Web client, ported from the Infinite Canvas single mother-material system (aurora stage, four depth layers, Tier A top-level interaction). The plugin is the single decision-maker: it works identically on stock DSH and on builds that consume its token contract.

## Screenshots

![Session view (backdrop aura on)](preview_1.png)
![Settings (backdrop aura on)](preview_2.png)
![Session view (pure black stage)](preview_3.png)
![Settings (backdrop aura off)](preview_4.png)
